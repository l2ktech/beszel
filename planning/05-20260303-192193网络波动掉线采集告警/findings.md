...
## 2026-03-08 DS224 运行状态与服务概览
- **发现**：`DS224+` 当前 DSM 版本为 `7.2.1-69057 Update 5`，Beszel 已在线（`status=up, ct=2`）；当前主要运行中的容器包括 `beszel-agent`、`zerotier`、`docker-registry`、`gitea`、`dify-on-wechat`。
- **来源**：SSH 登录群晖后执行系统信息、`docker ps` 与 Beszel 数据库查询。
- **影响**：群晖当前具备较多常驻服务，功能完整，但暴露面和维护复杂度较高。

## 2026-03-08 DS224 资源状态
- **发现**：系统盘 `/` 使用约 `20%`，数据卷 `/volume3` 使用约 `74%`；内存总量约 `17GiB`，可用约 `12GiB`。
- **来源**：`df -h`、`free -h`。
- **影响**：当前不存在系统盘或内存的即时瓶颈，但数据卷容量已进入需要持续关注的区间。

## 2026-03-08 DS224 可优化项
- **发现**：群晖上曾同时存在旧的本机版 `beszel-agent` systemd 服务与新容器版 agent；旧服务已停掉并禁用。另有 `Docker 20.10.23` 偏旧、SSH 仍启用密码登录、nginx 代理端口较多。
- **来源**：`systemctl status beszel-agent.service`、`docker --version`、`ss -tulpn`、SSH 配置抽查。
- **影响**：建议后续清理旧 agent 二进制/服务文件、逐步关闭 SSH 密码登录、评估不必要端口与旧运行时版本。

## 2026-03-08 DS224 安全审计结论
...
## 2026-03-08 ds224 自定义 193 探测映射
- **发现**：已在 `zt_latency_sync.sh` 中新增“按系统名覆盖 193 探测目标”的通用能力，并在运行时配置 `scripts/zt_alert.env` 中加入 `ZT_TARGET_193_MAP=ds224=192.168.193.188`。
- **来源**：脚本源码 `scripts/zt_latency_sync.sh` 与运行时配置 `scripts/zt_alert.env`。
- **影响**：`ds224` 后续不再走“`192.168.192.X -> 192.168.193.X` 机械推导”，而是固定按指定目标 `192.168.193.188` 探测。

- **发现**：手动执行脚本后，`ds224` 的 `zt_probe_ts` 已推进，说明自定义映射逻辑已实际生效；但 `z193=-1 / z193_status=down` 仍未恢复。
- **来源**：脚本执行后数据库查询。
- **影响**：问题已从“脚本探测目标错误”收敛为“目标地址本身不可达”。

- **发现**：本机对 `192.168.193.188` 的 `ICMP / 35622 / 9993` 全部失败；同时 `DS224` 的 `zerotier` 容器数据目录中仍只存在官方网络配置文件 `565799d8f61f7c2d.conf`，没有 `5cb1bf45e10c6865.conf`。
- **来源**：本机 `ping` / `nc -vz` 测试，DS224 上 `zerotier` 数据卷文件检查。
- **影响**：`ds224` 当前事实上**还没有真正加入 193 自建网络**；即使已经切换到自建 `planet`，没有完成 `join 5cb1bf45e10c6865` 并获得 `192.168.193.188` 之前，`z193_status` 仍会继续显示 `down`。

## 2026-03-08 ds224 真正加入 193 并恢复 z193_status=up
- **发现**：`DS224` 上原本同时存在宿主套件版 `zerotier` 与 Docker 版 `zerotier`，宿主进程占用 `9993`，导致 Docker 容器内部 CLI 一直命中错误实例；停掉宿主套件版 `zerotier` 后，Docker 容器才真正接管 `9993`。
- **来源**：DS224 主机进程检查、`synopkg status zerotier`、Docker 容器重启后的 `zerotier-cli` 验证。
- **影响**：这是之前 `join 193` 命令无效、`z193_status` 长期 `down` 的第一层根因。

- **发现**：`DS224` 的 Docker 版 `zerotier` 虽然已经挂载了 `/dev/net/tun`，但该设备初始权限为 `600 root:root`，而容器内 `zerotier-one` 进程实际以 UID `999` 运行，导致无法打开 TUN，日志持续报 `could not open TUN/TAP device`。
- **来源**：容器内 `/dev/net/tun` 权限检查、`docker top zerotier`、容器日志。
- **影响**：这是 `193` 接口起不来、`listnetworks` 空表的第二层根因。

- **发现**：将 DS224 主机上的 `/dev/net/tun` 改为 `666` 后，Docker 版 `zerotier` 成功同时挂起：
  - 官方网络 `565799d8f61f7c2d -> 192.168.192.100`
  - 自建网络 `5cb1bf45e10c6865 -> 192.168.193.188`
- **来源**：容器内 `zerotier-cli listnetworks`。
- **影响**：DS224 已真正加入 `193` 网络，并且拿到了固定地址 `192.168.193.188`。

- **发现**：通过控制器 API 已将节点 `16d72df3b4` 授权并固定为 `192.168.193.188`，名称设为 `ds224-self`。
- **来源**：`POST /api/network/5cb1bf45e10c6865/member/16d72df3b4` 返回 `200`。
- **影响**：控制面授权与静态地址已正式落库，不再是临时地址。

- **发现**：尽管 `4060 -> 192.168.193.188` 已稳定可达，但当前 `macmini-self` 到 `ds224` 的 193 peer 仍不稳定，因此为 `ds224` 增加了 `4060` 中继探测：`ZT_PROBE_RELAY_193_MAP=ds224=wzy@192.168.193.10:35622`。
- **来源**：本机与 `4060` 对 `192.168.193.188` 的对比连通性测试、运行时配置更新。
- **影响**：`z193_status` 现在反映的是“真实 193 可达性”，不会再被本机单点 peer 异常误判拖成 `down`。

- **发现**：最终手动执行 `zt_latency_sync.sh` 后，`ds224` 已恢复为：`z193=22ms`、`z193_status=up`、`z193_jitter=5`，且数据库 `quick_check=ok`。
- **来源**：脚本执行后数据库查询。
- **影响**：`DS224` 后面的 `193` 网络状态已从 `down` 变成真实的 `up`。
